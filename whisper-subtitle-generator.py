...
class WhisperSubtitleGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("Whisper Subtitle Generator (Nederlands)")
    ...
    def create_widgets(self):
        ...
        ttk.Label(model_frame, text="Uitvoer map:").grid(row=1, column=0, sticky=tk.W, pady=5)
        ttk.Entry(model_frame, textvariable=self.output_dir, width=50).grid(row=1, column=1, padx=5, pady=5)
        ttk.Button(model_frame, text="Bladeren...", command=self.browse_output_dir).grid(row=1, column=2, padx=5, pady=5)
        ...
        instructions_text = (
            ...
            "3. Selecteer optioneel een uitvoermap (standaard naast de video)\n"
            "4. Klik op 'Start' om Whisper te starten\n"
            "5. Wacht tot het proces is voltooid\n\n"
            "Let op: Dit programma vereist dat OpenAI Whisper en FFmpeg zijn geïnstalleerd.\n"
            "Installeer deze met: pip install openai-whisper ffmpeg-python"
        )
    ...
    def start_processing(self):
        ...
        threading.Thread(target=self.run_whisper, args=(video_path, output_dir, model_size), daemon=True).start()
    ...
    def run_whisper(self, video_path, output_dir, model_size):
        try:
            ...
            output_path = os.path.join(output_dir, f"{base_name}.nl.srt")
            ...
            # Voer transcriptie uit met Nederlands als doeltaal
            self.log("Transcriptie uitvoeren...")
            result = model.transcribe(
                video_path,
                language="nl",  # Nederlands forceren
                task="transcribe",
                verbose=False
            )
            ...
            # Hernoem bestand naar .nl.srt als dat nog niet is gedaan
